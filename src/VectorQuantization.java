...
public class VectorQuantization {
    ...
    static void Compress(int vectorHeight, int vectorWidth, int codeBlockSize, String Path) throws IOException{
        ...
        fileWriter.write("vector size : "+vectorWidth+" "+vectorHeight+"\n");
        fileWriter.write("[");
        for(int i=0;i<VectorsToQuantizedIndices.size();i++){
            fileWriter.write(VectorsToQuantizedIndices.get(i)+"");
            if(i+1 < VectorsToQuantizedIndices.size()) fileWriter.write(", ");
        }
        fileWriter.write("]\n");
        fileWriter.write("codeBlocks : [");
        for(int i=0;i<Quantized.size();i++){
            fileWriter.write("[");
            for(int j=0;j<Quantized.get(i).size();j++){
                fileWriter.write(Quantized.get(i).get(j)+"");
                if(j+1 < Quantized.get(i).size()) fileWriter.write(", ");
            }
            fileWriter.write("]");
            if(i+1 < Quantized.size()) fileWriter.write(", ");
        }
        fileWriter.write("]\n");
        fileWriter.close();
    ...
    private static void Quantize(int Level, Vector<Vector<Integer>> Vectors, Vector<Vector<Integer>> Quantized)
    {
        ...
        {
            if(Vectors.size() > 0) Quantized.add(vectorAverage(Vectors));
            return;
        }
        //Split
